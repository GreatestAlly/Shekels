...
{
    class InstructionsCardHoming : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int k = 3; k < 8 + player.extraAccessorySlots; k++)
            {
                if (player.armor[k].type == mod.ItemType("Sleeve") && crit && Main.rand.Next(4) != 0)
                {
                    ...
                    Projectile.NewProjectile(target.position.X, target.position.Y, (float)Math.Cos(offset) * 14f, (float)Math.Sin(offset) * 14f, mod.ProjectileType("InstructionsCardHoming"), projectile.damage, 1f, projectile.owner);
                }
            }
        }
