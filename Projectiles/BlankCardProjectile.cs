...
{
    class BlankCardProjectile : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int k = 3; k < 8 + player.extraAccessorySlots; k++)
            {
                ...
                {
                    Projectile.NewProjectile(player.position.X, player.position.Y, 0f, -9f, mod.ProjectileType("BlankCardHoming"), projectile.damage, 1f, projectile.owner);
                }
            }
        }
